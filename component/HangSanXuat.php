...
if (isset($_GET['p'])) {
    $p = $_GET['p'];
    $sql = "SELECT s.HinhURL, s.TenSanPham, s.MaSanPham, s.GiaSanPham, s.SoLuongXem FROM sanpham s JOIN loaisanpham l ON s.MaLoaiSanPham = l.MaLoaiSanPham JOIN hangsanxuat h ON h.MaHangSanXuat = s.MaHangSanXuat WHERE s.MaHangSanXuat = $p AND s.BiXoa = 0 AND h.BiXoa = 0 AND l.BiXoa = 0 AND s.SoLuongTon > 0";
} 
else {
    header('Location: ../');
}
